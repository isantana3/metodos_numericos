...
def gauss_seidel(base_matrix: list, error: float):
        ...
        for i in range(len(base_matrix)):
            ...
            for j in range(len(row) - 1):
                if i == j:
                    ...
                    x = x - (matrix[j] * actual_solution[j])
            x /= matrix[-1]
...
def run():
    data: TxtWorker = TxtWorker('gauss_seidel.txt')
    data.read_matrix()
    answers = []
    for i in range(len(data.matrix)):
        if converge(data.matrix[i]):
            answers.append(gauss_seidel(data.matrix[i], data.error[i]))
        else:
