...
def simpson_1_3(function: str, a: float, b: float, n: int = 2):
    h = b - a
    response = 0
    points = [a]
    fx = []
...
def simpson_3_8(function: str, a: float, b: float, n: int = 5):
    drain = n % 3
    h = b - a
    response = 0
    points = [a]
    ...
    partial = simpson_1_3(function, points[0], points[drain])
    h = b - points[drain]
    for i in range(n + 1):
        fx.append(solve_func(points[i], function))

    response = h * (fx[drain] + 3 * sum(fx[drain + 1 : -1]) + fx[-1]) / 8
    return f'Itegration of {function} is: {partial + response}'
...
def run():
    data: TxtWorker = TxtWorker('simpson_38.txt')
    data.read_function()
    answers = []
    for i in range(len(data.a)):
        answers.append(simpson_3_8(data.funcao[i], data.a[i], data.b[i]))
